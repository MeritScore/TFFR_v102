...
export const FanDashboard: React.FC<Props> = ({ userProfile }) => {
  ...
  useEffect(() => {
    const fetchBackendMessages = async () => {
      try {
        const backendMsgs = await api.getMessages('GLOBAL');
        setMessages(prev => {
          const existingIds = new Set(prev.map(m => m.id));
          const newMsgs = backendMsgs.filter((m: Message) => !existingIds.has(m.id));
    ...
    const pollInterval = setInterval(fetchBackendMessages, 5000);
    return () => clearInterval(pollInterval);
  }, []);

  const handleSendMessage = (text: string) => {
    // Fire and forget to Backend
    api.sendMessage({ text, senderId: CURRENT_USER.id, channel: 'GLOBAL' }).catch(e => console.error("Send failed", e));
