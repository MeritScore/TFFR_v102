...
export const ChatBubble: React.FC<ChatBubbleProps> = ({ message, isOwn, onTogglePause, onInteraction, onOpenThread, onReserve, onClaim, isPaused, hideActions = false }) => {
      ...
      <div className={`relative w-[98%] ${cardStyles} p-3 md:p-4 rounded-xl shadow-lg backdrop-blur-sm transition-all duration-300`}>
        <div className={`flex items-center gap-3 mb-2 pb-2 border-b border-white/10 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
            <div className="relative">
                {isHiveMind ? ( <div className="w-8 h-8 rounded-full bg-cyber-green flex items-center justify-center text-black shadow-neon-green"><Layers size={16} /></div> ) : ( <img src={user.avatarUrl || `https://ui-avatars.com/api/?name=${user.username}`} alt={user.username} className={`w-8 h-8 rounded-full border-2 ${isGig ? 'border-cyber-yellow' : isOwn ? 'border-cyber-green' : 'border-gray-500'}`} /> )}
                {!isOwn && !isHiveMind && ( <div className="absolute -bottom-1 -right-1 bg-black text-[8px] px-1 rounded-full border border-gray-600 text-gray-200 font-mono">{user.meritScore || 100}</div> )}
            </div>
            <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
                 <div className="flex items-center gap-1.5"><span className={`text-sm font-bold font-display ${isOwn ? 'text-cyber-green' : 'text-cyber-cyan'}`}>{user.username}</span>{user.isVerified && <Shield size={10} className="text-cyber-yellow fill-cyber-yellow" />}{isHiveMind && <span className="text-[10px] bg-cyber-green text-black px-1.5 rounded font-bold">x{repeatCount}</span>}</div>
