...
export const ChatBubble: React.FC<ChatBubbleProps> = ({ message, isOwn, onTogglePause, onInteraction, onOpenThread, onReserve, onClaim, onViewProfile, isPaused, hideActions = false }) => {
    ...
    const user = (typeof message.sender === 'object' ? message.sender : { id: 'unknown', username: 'Unknown User', meritScore: 0, avatarUrl: '' }) as User;
    ...
    const timeString = message.timestamp instanceof Date && !isNaN(message.timestamp.getTime())
            ...
            <div className={`relative w-[98%] ${cardStyles} p-3 md:p-4 rounded-xl shadow-lg backdrop-blur-sm transition-all duration-300`}>
                <div className={`flex items-center gap-3 mb-2 pb-2 border-b border-white/10 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div
                        ...
                        onClick={() => onViewProfile && onViewProfile(user)}
                    >
                        <div className="flex items-center gap-1.5"><span className={`text-sm font-bold font-display transition-colors ${isOwn ? 'text-cyber-green group-hover/name:text-white' : 'text-cyber-cyan group-hover/name:text-white'}`}>{user.username || 'Unknown'}</span>{user.isVerified && <Shield size={10} className="text-cyber-yellow fill-cyber-yellow" />}{isHiveMind && <span className="text-[10px] bg-cyber-green text-black px-1.5 rounded font-bold">x{repeatCount}</span>}</div>
                        <span className="text-[10px] text-gray-400 font-mono uppercase">{timeString}</span>
                    </div>
